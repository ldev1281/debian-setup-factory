...
bitwarden::get_secret() {
    ...
    if [[ "$count" -eq 0 ]]; then
        echo "No secret found with key \"$key\"" >&2
        if [[ -t 0 || -t 1 ]]; then
            local value reply
            ...
            if [[ "$reply" =~ ^[Yy]$ || "$reply" =~ ^[Yy][Ee][Ss]$ ]]; then
                if [[ -n "$project_id" ]]; then
                    bitwarden::create_secret "$project_id" "$key" "$value" \
                        || echo "Failed to create secret \"$key\" in project \"$project_id\"" >&2
                else
                    echo "Project ID was not provided to bitwarden::get_secret; skipping save." >&2
                fi
            fi
        ...
        echo "Multiple secrets with key \"$key\"; refine (pass project id) or use --first" >&2
        return 1
    fi
